...
SELECT e.nome AS Nome_Editora, l.Titulo
FROM Editora e
JOIN Livro l ON e.idEditora = l.idEditora
ORDER BY e.nome DESC;

SELECT e.nome AS Nome_Editora, AVG(l.preco) AS Media_Preço
FROM Editora e
JOIN Livro l ON e.idEditora = l.idEditora
GROUP BY e.nome;

SELECT c.nome AS Nome_Cliente, 
       COALESCE(SUM(ip.quantidade), 0) AS Quantidade_Livros_Comprados
FROM Cliente c
